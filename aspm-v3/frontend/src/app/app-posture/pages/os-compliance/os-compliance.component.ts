...
@Component({ selector: 'app-os-compliance', templateUrl: './os-compliance.component.html' })
export class OsComplianceComponent implements OnInit {
  ...
  ngOnInit(){this.load();}
  load(){this.loading=true;this.svc.getFindings('compliance',{page:this.page,limit:this.limit,compType:'os',...this.filters}).subscribe({next:r=>{this.findings=r.data||r;this.total=r.meta?.total||this.findings.length;this.loading=false;},error:()=>this.loading=false});}
