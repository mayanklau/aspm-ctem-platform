...
@Component({ selector: 'app-bas-detail', templateUrl: './bas-detail.component.html' })
export class BasDetailComponent implements OnInit {
  ...
  load() {
    this.loading = true;
    this.svc.getFindings('bas', { page:this.page, limit:this.limit, ...this.filters }).subscribe({
      next: r => { this.findings = r.data || r; this.total = r.meta?.total || this.findings.length; this.loading = false; },
      error: () => this.loading = false
    });
  }
  loadSummary() { this.svc.getSummary('bas', { appId:this.filters.appId }).subscribe({ next: d => this.summary = d, error: () => {} }); }
  ...
  updateStatus(id: string, status: string) { this.svc.updateStatus('bas', id, status).subscribe({ next: () => this.load() }); }
  ...
  exportCsv() {
    ...
    const a = document.createElement('a'); a.href='data:text/csv;charset=utf-8,'+encodeURIComponent(csv); a.download='bas_findings.csv'; a.click();
  }
