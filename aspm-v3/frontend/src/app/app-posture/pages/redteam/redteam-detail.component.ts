...
@Component({ selector: 'app-redteam-detail', templateUrl: './redteam-detail.component.html' })
export class RedteamDetailComponent implements OnInit {
  ...
  load() {
    this.loading = true;
    this.svc.getFindings('redteam', { page:this.page, limit:this.limit, ...this.filters }).subscribe({
      next: r => { this.findings = r.data || r; this.total = r.meta?.total || this.findings.length; this.loading = false; },
      error: () => this.loading = false
    });
  }
  loadSummary() { this.svc.getSummary('redteam', { appId:this.filters.appId }).subscribe({ next: d => this.summary = d, error: () => {} }); }
  ...
  updateStatus(id: string, status: string) { this.svc.updateStatus('redteam', id, status).subscribe({ next: () => this.load() }); }
  ...
  exportCsv() {
    ...
    const a = document.createElement('a'); a.href='data:text/csv;charset=utf-8,'+encodeURIComponent(csv); a.download='redteam_findings.csv'; a.click();
  }
