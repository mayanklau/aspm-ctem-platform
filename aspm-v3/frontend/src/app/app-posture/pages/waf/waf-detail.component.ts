...
@Component({ selector: 'app-waf-detail', templateUrl: './waf-detail.component.html' })
export class WafDetailComponent implements OnInit {
  ...
  load() {
    this.loading = true;
    this.svc.getFindings('waf', { page: this.page, limit: this.limit, ...this.filters }).subscribe({
      next: r => { this.findings = r.data || r; this.total = r.meta?.total || this.findings.length; this.loading = false; },
      error: () => this.loading = false
    });
  }
  loadSummary() { this.svc.getSummary('waf', { appId: this.filters.appId }).subscribe({ next: d => this.summary = d, error: () => {} }); }
  ...
  updateStatus(id: string, status: string) { this.svc.updateStatus('waf', id, status).subscribe({ next: () => this.load() }); }
