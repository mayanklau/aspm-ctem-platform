...
@Component({ selector: 'app-pt-external', templateUrl: './pt-external.component.html' })
export class PtExternalComponent implements OnInit {
  ...
  load(){this.loading=true;this.svc.getFindings('pt',{page:this.page,limit:this.limit,ptType:'external_webapp',...this.filters}).subscribe({next:r=>{this.findings=r.data||r;this.total=r.meta?.total||this.findings.length;this.loading=false;},error:()=>this.loading=false});}
